...
import { DeviceItem, ToolMappingItem, SensorItem, CategoryItem, PlantItem } from '../../types';
...
export const DeviceSetup: React.FC<DeviceSetupProps> = ({
  ...
  const [deviceName, setDeviceName] = useState('PowerCommand Cloud Edge Node');
  const [imei, setImei] = useState('356789104590731');
  const [vendor, setVendor] = useState('Eicher Motors');
  const [selectedPlant, setSelectedPlant] = useState(plants[0]?.name || '');

  const [manualProtocol, setManualProtocol] = useState('MODBUS TCP');
  ...
  // Sync initial plant if state was empty
  useEffect(() => {
  ...
  // Check if the Device Name matches any Admin Tool Mapping by name
  const matchedTool = useMemo(() => {
    if (!deviceName.trim()) return null;
    return toolMappings.find(
      (t) => t.toolName.trim().toLowerCase() === deviceName.trim().toLowerCase()
    ) || null;
  }, [deviceName, toolMappings]);

  // Whenever the Device Name matches (or stops matching) an Admin Tool Mapping,
  // auto-populate (or clear) its sensors and parameters
  useEffect(() => {
    if (matchedTool) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newDevice: DeviceItem = {
      ...
      vendor: vendor,
      status: 'Online',
      toolProfile: matchedTool ? matchedTool.toolName : 'Custom Tool Configuration',
      plant: selectedPlant,
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xs overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm font-sans">
            {/* Device Name (also matched against Admin Tool Mappings to auto-fetch sensors) */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {matchedTool ? (
                  <span className="text-[11px] font-medium text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                    ...
                    <span>Matches Admin Tool Mapping</span>
                  </span>
                ) : (
                  <span className="text-[11px] font-medium text-slate-400">No Tool Mapping match</span>
                )}
              </div>
              <input
                type="text"
                required
                list="admin-tool-mapping-options"
                value={deviceName}
                onChange={(e) => setDeviceName(e.target.value)}
                placeholder="e.g. GenTool v3.2 Heavy Suite"
                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors font-sans"
              />
              <datalist id="admin-tool-mapping-options">
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                Matching this name to an Admin Tool Mapping auto-fetches its sensors and parameters below.
              </p>
            </div>
            ...
            {/* Plant Dropdown (Strictly from Admin Plants) */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">
              ...
              >
                {plants.length === 0 ? (
                  <option value="">No plants configured in Admin</option>
                ) : (
                  plants.map((p) => (
            ...
            <div>
              ...
              >
                ...
                <option value="Bosch IoT">Bosch IoT</option>
                <option value="Cummins Telematics">Cummins Telematics</option>
              </select>
            </div>
          </div>

          {/* Tool Mapping Match (driven by Device Name) */}
          <div className="pt-5 border-t border-slate-200 dark:border-slate-800 space-y-3">
            {/* Quick-select chips from existing Admin Tool Mappings, sets Device Name directly */}
            <div className="space-y-1.5">
              <div className="text-xs text-slate-500 dark:text-slate-400">
                Quick Select an Admin Tool Mapping (sets Device Name):
              </div>
              <div className="flex flex-wrap gap-1.5">
                {toolMappings.map((tool) => {
                  const isCurrent = deviceName.trim().toLowerCase() === tool.toolName.trim().toLowerCase();
                  return (
                    <button
                      key={tool.id}
                      type="button"
                      onClick={() => setDeviceName(tool.toolName)}
                      className={`px-3 py-1 text-xs rounded-lg border transition-all cursor-pointer flex items-center gap-1.5 ${
                        isCurrent
                          ? 'bg-sky-100 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300 border-sky-300 dark:border-sky-700 font-medium shadow-xs'
                          : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'
                      }`}
                    >
                      {isCurrent && <Check className="w-3 h-3 text-sky-600 dark:text-sky-400" />}
                      <span>{tool.toolName}</span>
                      <span className="text-[10px] text-slate-400">({tool.mappedSensors.length} sensors)</span>
                    </button>
                  );
                })}
              </div>
            ...
            {matchedTool ? (
              <div className="p-4 bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50 rounded-lg space-y-3">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300">
                  The Device Name matches an Admin Tool Mapping. The mapped sensors and telemetry parameters have been auto-populated below:
                </p>
              ...
              /* UNMATCHED CASE: Show empty notice, user selects manually */
              <div className="p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-lg flex items-start gap-3">
                ...
                <div className="text-xs">
                  <div className="font-semibold text-slate-800 dark:text-slate-200">
                    {deviceName.trim()
                      ? `No Tool Mapping found for "${deviceName}"`
                      : 'Device Name is Empty (Manual Configuration Mode)'}
                  </div>
            ...
            {activeSensorNames.length === 0 ? (
              <div className="py-8 text-center border border-dashed border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
                  Sensors are empty because the Device Name does not match an Admin Tool Mapping. Click any sensor below to manually assign it and its parameters.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 truncate mb-2">
                        {sensor.industryType} • {sensor.protocol || 'MODBUS'}
                      </div>
                    </div>
